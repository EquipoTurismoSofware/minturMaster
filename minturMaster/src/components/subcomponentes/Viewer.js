...
class Viewer extends Component {
  ...
  setPagina(nro_pagina) {
    let mostrar_desde =
      nro_pagina * this.state.preset.items_visibles -
      (this.state.preset.items_visibles - 1);
  ...
  componentDidUpdate(preProps) {
    if (
      this.state.preset.total_items !==
        React.Children.count(this.props.children) ||
      this.props.visibles !== preProps.visibles
    ) {
      this.calcular();
    }
  }
    ...
    for (let i = 1; i < this.state.preset.total_paginas + 1; i++) {
      if (i === this.state.preset.pagina_actual) {
        paginas.push(
          <li
            key={`btn-pag-${i}`}
            style={this.state.paginationVisibility[i - 1]}
            className="selected"
            onClick={(e) => this.setPagina(i)}
          >
            {i}
          </li>
        );
    ...
    const childrens = React.Children.map(
      ...
      (children, index) => {
        ...
        return (
          <div
            key={`d-${index}`}
            style={this.state.visibility[i]}
            className="animated bounceInRight children"
          >
            {children}
          </div>
        );
      ...
      <React.Fragment>
        {this.state.loading ? (
          ...
          <div className="" style={{ textAlign: "center", marginTop: "100px" }}>
            <h5 style={{ color: `#722789` }}>No se encontraron elementos.</h5>
          </div>
          ...
          <div className="Viewer">
            ...
            <div className="itemsChildren">{childrens}</div>
            ...
            <div className="pages">
              ...
              <ul style={{ textAlign: "center" }}>{paginas}</ul>
